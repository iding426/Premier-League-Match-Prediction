...
class FPLMatchPredictor(nn.Module):
    ...
    def __init__(
        ...
        self.team_pos_emb = nn.Parameter(torch.randn(1, 20, model_dim))   # up to 20 team tokens
        self.match_pos_emb = nn.Parameter(torch.randn(1, 1, model_dim))   # match info token

        # Team leval transformer
        ...
        self.classifier = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(model_dim, 3)  # win / draw / loss
        )
    ...
    def forward(self, team1_tokens, team2_tokens, match_features):
        """
        Inputs:
            team1_tokens: (B, T1, input_dim_team)
            team2_tokens: (B, T2, input_dim_team)
            match_features: (B, input_dim_match)

        T1 and T2 are sequence lengths of the team feature tokens.
        """
        ...
        t1_encoded = self.team_encoder(t1)      # (B, T1, model_dim)
        t2_encoded = self.team_encoder(t2)      # (B, T2, model_dim)

        # Match projection + positional embedding
        m = self.match_proj(match_features).unsqueeze(1)   # (B,1,model_dim)
